...
namespace Net {
    ...
    static bool parseUrl(const std::string& url, std::string& host, int& port, std::string& path) {
        size_t pos = 0;
        if (url.find("http://") == 0) { pos = 7; port = 80; }
        else if (url.find("https://") == 0) { pos = 8; port = 443; }
        else return false;

        size_t pathPos = url.find('/', pos);
        size_t colonPos = url.find(':', pos);

        if (pathPos == std::string::npos) { path = "/"; pathPos = url.length(); }
        else path = url.substr(pathPos);

        if (colonPos != std::string::npos && colonPos < pathPos) {
            host = url.substr(pos, colonPos - pos);
            try { port = std::stoi(url.substr(colonPos + 1, pathPos - colonPos - 1)); } catch (...) { return false; }
        } else host = url.substr(pos, pathPos - pos);
    ...
    static std::string makeHttpRequest(const std::string& host, int port, const std::string& requestStr, bool& success, std::string& errorMsg) {
        ...
        if (mbedtls_net_connect(&fd, host.c_str(), std::to_string(port).c_str(), MBEDTLS_NET_PROTO_TCP) != 0) {
            errorMsg = "Connection failed"; success = false; return "";
        }
        ...
        if (port == 443) {
            ...
            if (mbedtls_ctr_drbg_seed(&ctr_drbg, mbedtls_entropy_func, &entropy, (const unsigned char*)"trypillia", 9) != 0 ||
                mbedtls_ssl_config_defaults(&conf, MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM, MBEDTLS_SSL_PRESET_DEFAULT) != 0) {
                errorMsg = "SSL setup failed"; success = false; return "";
            }
            ...
            int ret;
            while ((ret = mbedtls_ssl_read(&ssl, buf, sizeof(buf))) > 0) response.append((char*)buf, ret);
            ...
            unsigned char buf[4096];
            int ret;
            while ((ret = mbedtls_net_recv(&fd, buf, sizeof(buf))) > 0) response.append((char*)buf, ret);
            mbedtls_net_free(&fd);
            success = true; return response;
        }
    }
    ...
    static VMValue httpGet(int argCount, VMValue* args) {
        ...
        if (!parseUrl(std::get<std::string>(args[0]), host, port, path)) return makeResultErr(currentVM, "Invalid URL");
    ...
    struct SocketData { mbedtls_net_context fd; };
    static void freeSocket(void* data) {
        if (data) { mbedtls_net_free(&((SocketData*)data)->fd); delete (SocketData*)data; }
    }
    ...
    static VMValue socketConnect(int argCount, VMValue* args) {
        ...
        if (mbedtls_net_connect(&data->fd, std::get<std::string>(args[0]).c_str(), std::to_string((int)std::get<double>(args[1])).c_str(), MBEDTLS_NET_PROTO_TCP) != 0) {
            delete data; return makeResultErr(currentVM, "Connect failed");
        }
    ...
    static VMValue socketClose(int argCount, VMValue* args) {
        auto inst = std::get<std::shared_ptr<ObjInstance>>(args[-1]);
        if (inst->nativeData) { mbedtls_net_free(&((SocketData*)inst->nativeData)->fd); inst->nativeData = nullptr; }
    ...
    void registerSymbols(SymbolTable* scope) {
        Symbol s; s.type = "class"; s.isConst = true;
        s.name = "Http"; scope->define(s);
        s.name = "Socket"; scope->define(s);
    }
}
}
