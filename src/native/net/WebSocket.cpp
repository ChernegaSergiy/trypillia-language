...
namespace WebSocketModule {
    ...
    struct WSServerData { mbedtls_net_context fd; };
    struct WSClientData { mbedtls_net_context fd; };

    static void freeServer(void* data) {
        if (data) { mbedtls_net_free(&((WSServerData*)data)->fd); delete (WSServerData*)data; }
    }

    static void freeClient(void* data) {
        if (data) { mbedtls_net_free(&((WSClientData*)data)->fd); delete (WSClientData*)data; }
    }
    ...
    static VMValue wsServerCreate(int argCount, VMValue* args) {
        ...
        auto inst = std::make_shared<ObjInstance>(std::get<std::shared_ptr<ObjClass>>(currentVM->globals["WebSocketServer"]));
        inst->nativeData = data; inst->freeFn = freeServer;
    ...
    static VMValue wsServerAccept(int argCount, VMValue* args) {
        ...
        if (ret <= 0) { mbedtls_net_free(&client_fd); return makeResultErr(currentVM, "Handshake read failed"); }
        ...
        if (wsKey.empty()) {
            ...
            mbedtls_net_free(&client_fd); return makeResultErr(currentVM, "Not a websocket");
        }
        ...
        wsInst->nativeData = clientData; wsInst->freeFn = freeClient;
    ...
    static VMValue wsSend(int argCount, VMValue* args) {
        ...
        if (len <= 125) frame.push_back((uint8_t)len);
    ...
    static VMValue wsRecv(int argCount, VMValue* args) {
        ...
        WSClientData* data = (WSClientData*)inst->nativeData;
        uint8_t header[2];
        if (mbedtls_net_recv(&data->fd, header, 2) <= 0) return makeResultErr(currentVM, "Closed");
    ...
    void registerSymbols(SymbolTable* scope) {
        ...
        s.name = "WebSocketServer"; scope->define(s);
        s.name = "WebSocket"; scope->define(s);
    }
}
}
